...
const team_id = "7933" //add new league id
const type = 4
...
const list_notification_path = {
    "Post": "/posts_view",
    "Match": "/match/match_view",
    "Post-Path": "/posts_view/",
    "Match-Path": "/match/match_view/" 
}

const sendSSE = async (type: string, channel?: string, path?: string, full_path?: string, param?: string, title?: string, text?: string, image?: string, notification_layout?: string) => {
    let json_template = { 
        "type": type,
        "channel": channel,
        "path": path,
        ...
        "text": text,
        "image": image,
        "notification-layout": notification_layout,
    }
...
const live_match = async (h, m, fix_id, search_data) =>  {
    ...
    await axios.get(url ,{headers: headers})
        .then(async (response) => {
            ...
            const add_match = Matches.create({
                ...
                status_long: fixtures_status[x.fixture.status.short],
                home_team:
                    {id: x.teams.home.id, name: x.teams.home.name, logo: x.teams.home.logo, goals: x.goals.home},
                
                away_team:
                    {id: x.teams.away.id, name: x.teams.away.name, logo: x.teams.away.logo, goals: x.goals.away},
    ...
    while (live == false) {
        console.log("While")
        const date = new Date()
        let today = new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDay(), date.getHours(), date.getMinutes()))//new Date(new Date().toLocaleTimeString('en-US', { timeZone: '"Europe/Prague"' }))
        ...
        if (today >= starting_time) {
            ...
            .then(async (response) => {
                ...
                if (x.fixture.status.short != "NS") {
                    ...
                    await sleep(30000) // 1 min
                }
            })
        }
        else {
            if (getLineup == false) {
                if (today >= lineups_time) { //Maybe has
                    await axios.get(url ,{headers: headers})
                    .then(async (response) => {
                        if (response.data.response.length != 0) {
                            ...
                            if (x.lineups.length != "") {
                                ...
                                let home_formation = x.lineups[0].formation
                                let home_color = x.lineups[0].team.colors.player.primary
                                let away_color = x.lineups[0].team.colors.player.primary
                                let away_formation = x.lineups[1].formation
                                for (var hl of x.lineups[0].startXI) {
                                    let player: player = {id: hl.player.id, name: hl.player.name, number: hl.player.number, position: hl.player.pos, grid: hl.player.grid}
                                    home_lineup.push(player)
                                }
                                for (var hs of x.lineups[0].substitutes) {
                                    let player: player = {id: hs.player.id, name: hs.player.name, number: hs.player.number, position: hs.player.pos, grid: hs.player.grid}
                                    home_substitutes.push(player)
                                }
                                for (var al of x.lineups[1].startXI) {
                                    let player: player = {id: al.player.id, name: al.player.name, number: al.player.number, position: al.player.pos, grid: al.player.grid}
                                    away_lineup.push(player)
                                }
                                for (var as of x.lineups[1].substitutes) {
                                    let player: player = {id: as.player.id, name: as.player.name, number: as.player.number, position: as.player.pos, grid: as.player.grid}
                                    away_substitutes.push(player)
                                }
                                await Matches.update({
                ...
                if (x.fixture.status.short == "FT") {
                    ...
                    if (x.goals.home != current_match.home_team.goals || x.goals.away != current_match.away_team.goals || x.fixture.status.short != current_match.status_short || x.fixture.status.elapsed != parseInt(match_elapsed)) {
                        ...
                        if (home == true && x.goals.home > current_match.home_team.goals || home == false && x.goals.away > current_match.away_team.goals) {
                            ...
                            await sendSSE("notification", list_notification_channel["Live"], list_notification_path["Match"], list_notification_path["Match-Path"] + "/", fix_id.toString(), notification_title, "Text", "Image", "NotificationLayout.Default")
                        }
                        if (home == true && x.goals.home < current_match.home_team.goals || home == false && x.goals.away < current_match.away_team.goals) {
                            //Dostali jsme gol
...
const match_checker = async () => {
    ...
    if (found == null) {
        ...
        let notification_text = `Nezapomeňte na dnešní zápas Slavie proti ${found.away_team} v ${found.place}. Začátek utkání je naplánován na ${found.time.hour}:${found.time.minutes}, takže si připravte svoje oblíbené nápoje a jídlo a připojte se k nám při sledování tohoto skvělého zápasu. Podpořme náš tým a věřme, že získají vítězství!` 
        ...
        */
        await sendSSE("notification", list_notification_channel["Match"], list_notification_path["Match"], list_notification_path["Match-Path"] + "/", found.fixture_id.toString(), notification_title, notification_text, "Image", "NotificationLayout.BigText")
