...
router.post("/getmatches", async (req: Request, res: Response) => {
    ...
    let response = []
    let find = await Matches.find({where: {created_date: MoreThan(new Date(body.last_updated))}})//await Matches.findOne({where: {fixture_id: 962767}})
    for (var x of find) {
...
router.post("/getfuturematches", async (req: Request, res: Response) => {
    ...
    let response = await FutureMatch.find({where: {created_date: MoreThan(new Date(body.last_updated))}})//await Matches.findOne({where: {fixture_id: 962767}})
    /*let events = await Events.find({where: {fixture_id: find.fixture_id}})
        let complete = {
            "match": find,
            "events": events
...
router.post("/getposts", async (req: Request, res: Response) => {
    ...
    let response = await Posts.find({where: {created_date: MoreThan(new Date(body.last_updated))}})//await Matches.findOne({where: {fixture_id: 962767}})
    /*let events = await Events.find({where: {fixture_id: find.fixture_id}})
        let complete = {
            "match": find,
            "events": events
...
router.get("/getvideos", async (req: Request, res: Response) => {
    console.log("Videos")
    let response = await Videos.find()//await Matches.findOne({where: {fixture_id: 962767}})
    /*let events = await Events.find({where: {fixture_id: find.fixture_id}})
        let complete = {
            "match": find,
            "events": events
